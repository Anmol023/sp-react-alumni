...
import { faCheckCircle, faFastBackward, faFastForward, faList, faPlusSquare, faStepBackward, faStepForward, faTrash } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import MyToast from './MyToast'

export default class User_AddFriend extends Component{
    constructor(props){
        super(props);
        this.state = {
            alumni: [],
    ...
    findAllUser(){
        ...
        axios.get('http://localhost:8080/user/', {headers:{"Authorization": jwtToken}})
    ...
    sendRequest = (email2) =>{
        ...
        axios.post("http://localhost:8080/user/friendRequest",input, {headers:{"Authorization": jwtToken}})
            .then(response =>{
        ...
        return(
            <div>
                <div styles = {{display: this.state.show ?"block":"none"}}>
                    <MyToast show = {this.state.show} message = {"Request Sent."} type = {"success"}/>
                </div>
                <Card className="border border-dark bg-dark text-white ">
                <Card.Header><FontAwesomeIcon icon={faList}/>Send Request</Card.Header>
                <Card.Body>
                                ...
                                currrentUser.map((a, index)=>(
                                    <tr key={index}>
                                        ...
                                        <td>
                                            ...
                                            <Button size="block" variant="outline-primary" onClick={this.sendRequest.bind(this, a.email)}><FontAwesomeIcon icon={faPlusSquare} /></Button>
