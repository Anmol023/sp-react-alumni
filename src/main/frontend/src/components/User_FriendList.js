...
import { faCheckCircle, faFastBackward, faFastForward, faList, faStepBackward, faStepForward, faTrash } from '@fortawesome/free-solid-svg-icons';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import MyToast from './MyToast'

export default class User_FriendList extends Component{
    constructor(props){
        super(props);
        this.state = {
            alumni: [],
    ...
    allFriend = () =>{
        ...
        axios.post("http://localhost:8080/user/getFriendsList",input, {headers:{"Authorization": jwtToken}})
    ...
    deleteUser = (email2) =>{
        ...
        axios.post("http://localhost:8080/admin/",input, {headers:{"Authorization": jwtToken}})
        .then(response => {
            if(response.data!= null){
                this.setState({'show': true,'method': 'del'})
                setTimeout(() => this.setState({'show': false}), 3000)
                this.setState({
                    alumni: this.state.alumni.filter(a => a.userName !== email2)
                })
            }else{
    ...
    render(){
        ...
        return(
            <div>
                ...
                <Card.Header><FontAwesomeIcon icon={faList}/> All Friends</Card.Header>
                <Card.Body>
                                ...
                                currrentUser.map((a, index)=>(
                                    <tr key={index}>
                                        ...
                                        <td>
                                            ...
                                            <Button size ="sm" variant="outline-danger" onClick={this.deleteUser.bind(this, a.email)}><FontAwesomeIcon icon={faTrash} /></Button>
